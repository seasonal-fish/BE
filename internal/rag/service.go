...
(B) 대중 정서 민감성 — (A)는 아니지만 실재하는 사회적 고통·약자를 건드려 상당수 대중이 불쾌·부적절하다 느낄 소지. 예) 경제적 고통(빚·취업난·주가 폭락·영끌·텅장), 질병·죽음·사고, 약자·소수자 비하, 집단 고정관념(여성=약함·의존, 성역할 규정, 세대·지역 비하 등)의 재생산, 특정 대상(개인이든 집단이든)을 조롱·멸시·비꼬아 깎아내리거나 우스꽝스럽게·열등하게 그리는 어조, 대상의 노력·성취·처지를 헛되거나 하찮은 것으로 규정하는 어조. (예시는 망라가 아니라 원리를 보이는 것이다.)
[조롱·멸시 판별] 위 '조롱·멸시'는 대상을 '깎아내릴' 때만이다. '깎아내린다'에는 대상의 노력·성취·처지를 헛되거나 하찮은 것으로 규정하는 것도 포함한다(예: 열심히 한 일을 '헛수고'로, 성취·분투를 '별것 아님'으로 틀 짓기) — 이런 문구는 위트·말장난·언어유희의 외피를 써도 needs_review 다. 반대로 대상을 깎아내리지 않는 순수한 위트·도발·자신감·능청 톤(예: 도발적 질문, 너스레)은 안전이다 — 어조가 세다는 이유만으로 표시하지 마라.
...
- 다만 다음 명백한 false positive 는 표시하지 않는다: (1) 단어가 우연히 부정적·자극적 어감만 띨 뿐 실제 민감 대상을 지칭·전제하지 않는 단순 단어 매칭, (2) 관용적 과장·일상어·개인사·순수 제품/판촉 표현, (3) 대상을 깎아내리지 않는 위트·도발·자신감·능청 톤(단, 대상의 노력·성취·처지를 헛되거나 하찮은 것으로 규정하면 위트라도 여기서 제외 — [조롱·멸시 판별] 참조). 이 셋은 '민감 소지'가 아니라 표면적 겹침이다.
- 경제적 고통(주식·영끌·텅장·빚·취준 등), 질병·죽음·사고, 약자·소수자 비하, 집단 고정관념(성역할·세대·지역 등)을 가볍게 건드리면 needs_review 로 표시한다(위험 아님).
- 성역할 고정관념은 대상을 깎아내리지 않아도, 긍정·애정·응원 어조여도 표시한다(needs_review). 성별로 역할·기질을 분담·전제·재생산하면 대상이다 — 예: 엄마=가사·돌봄·희생 / 아빠=생계·가장·강인함 ("엄마 쉬세요, 아빠 힘내세요"는 돌봄·휴식은 엄마 몫, 부양·분투는 아빠 몫이라는 성별 역할 분담을 전제한다). 특정 성별을 비하하지 않더라도 '그 역할이 그 성별의 몫'이라는 전제를 재생산하는 것 자체가 (B) 대상이며, 다정한 판촉 문구라는 이유로 면제하지 마라.
